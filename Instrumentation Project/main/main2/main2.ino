...
void readSensor()
{
  lower_read=analogRead(low_level_pin);
  upper_read=analogRead(upper_level_pin);
  delay(100);
}
    ...
    if(upper_read>sensitivity)
    {
      ...
      if(upper_read>sensitivity)
      {
        ...
        if(upper_read>sensitivity)
        {
          ...
          if(upper_read>sensitivity)
          {
            Serial.println("bottle over filled");
            availablePerc=100;
          }
        }
      }
...
delay(9000);
  ...
  lcdCharDisplay(2,1,"Type 0 to close",0,1);
    initiateLInput:
  char keyL = keypad.getKey();
  if (keyL){
    if(searchInArray(inputOption_0,6,getKeypadInteger(keyL)))
    {
      true;
    }
    else
...
void releaseToPatient()
{
  ...
  lcdCharDisplay(2,1,"Type 0 to close",0,1);
  initiateRInput:
  char keyR = keypad.getKey();
  if (keyR){
    if(searchInArray(inputOption_0,6,getKeypadInteger(keyR)))
    {
      loop();
    }
    else
    {
      warnInvalidInput();
      lcdCharDisplay(2,0,"Selline Released",1,1);
      Serial.println("Selline Released!!");
      lcdCharDisplay(2,1,"Type 0 to close",0,1);
      goto initiateRInput;
    }
  }
...
int clean()
{
  ...
  if(waste_removed==1)
  {
    ...
    lcdCharDisplay(2,1,"Type 0 to close",0,1);
    int input=takeIntegerInput(inputOption_0);
  }
...
//  {
//    processingMessage(3,"Loading Cleaner");
//    turnOnMotor(waterM);
//    mesaureWaterPerc();
//  }
processingMessage(3,"Loading Cleaner");
...
//  while(availablePerc>lowerLevel)
//  {
//    processingMessage(3,"Removing Cleaner");
//    turnOnMotor(wasteM);
//    mesaureWaterPerc();
//  }
//  }
